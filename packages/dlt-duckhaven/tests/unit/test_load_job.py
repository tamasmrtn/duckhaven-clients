"""DuckHavenCopyJob: stage the local file then INSERT via read_parquet; use the resolved
remote path for an external-staging reference job; never put a credential in the load SQL."""
...
def test_local_file_is_staged_then_inserted(monkeypatch):
    sql_client = _sql_client()
    remote = "s3://bucket/sales/_staging/load1/orders.abc.0.parquet"
    stage = MagicMock(return_value=remote)
    monkeypatch.setattr(load_jobs._staging, "stage_file", stage)

    _job("/tmp/orders.abc.0.parquet", sql_client).run()

    stage.assert_called_once_with(
        sql_client.native_connection, "/tmp/orders.abc.0.parquet", "load1"
    )
    sql = sql_client.execute_sql.call_args.args[0]
    assert sql.startswith('INSERT INTO "raw"."analytics"."orders" BY NAME')
    assert f"read_parquet('{remote}', union_by_name=true)" in sql
...
def test_jsonl_uses_read_json(monkeypatch):
    ...
    monkeypatch.setattr(
        load_jobs._staging,
        "stage_file",
        MagicMock(return_value="s3://bucket/_staging/load1/orders.abc.0.jsonl"),
    )

    _job("/tmp/orders.abc.0.jsonl", sql_client).run()

    sql = sql_client.execute_sql.call_args.args[0]
    assert "read_json('s3://bucket/_staging/load1/orders.abc.0.jsonl')" in sql
